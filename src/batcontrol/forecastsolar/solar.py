...
class ForecastSolar:
    ...
    def create_solar_provider(
        ...
        cache_manager, http_client, rate_limit_manager = ForecastSolar._get_shared_infrastructure()
        logger.debug(f"Creating {requested_provider} provider with shared infrastructure")
        ...
        if provider is None:
            raise RuntimeError(f'[ForecastSolar] Failed to create provider for {requested_provider}')
        
        logger.info(f"Created {requested_provider} solar provider with shared infrastructure")
        return provider
