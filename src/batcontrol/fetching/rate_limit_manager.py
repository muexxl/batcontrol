...
from .constants import RATE_LIMIT_HEADERS, RATE_LIMIT_BACKOFF_FACTOR
...
class RateLimitManager:
    ...
    def is_rate_limited(self, provider_id: str) -> bool:
        ...
        with self._lock:
            ...
            if current_time >= rate_limit_info.created_at + rate_limit_info.retry_after:
                ...
                logger.info(f"Rate limit expired for provider: {provider_id}")
                return False
            
            remaining_time = (rate_limit_info.created_at + rate_limit_info.retry_after) - current_time
            logger.debug(f"Provider {provider_id} is rate limited for {remaining_time:.1f} more seconds")
            return True
    ...
    def set_rate_limit_from_response(self, provider_id: str, response) -> Optional[RateLimitInfo]:
        """
        ...
        if 'X-Ratelimit-Retry-At' in headers:
            ...
            try:
                ...
                retry_after = (retry_at_timestamp - datetime.datetime.now().astimezone()).total_seconds()
                rate_limit_info = RateLimitInfo(retry_after, retry_at_timestamp)
                logger.info(f"Parsed X-Ratelimit-Retry-At for {provider_id}: retry after {retry_after:.1f}s")
            except (ValueError, TypeError) as e:
                logger.warning(f"Failed to parse X-Ratelimit-Retry-At header: {e}")
        ...
        # Check for standard Retry-After header
            ...
            try:
                ...
                if retry_after_str.isdigit():
                    ...
                    retry_after = (retry_at_timestamp - datetime.datetime.now()).total_seconds()
                
                rate_limit_info = RateLimitInfo(retry_after)
                logger.info(f"Parsed Retry-After for {provider_id}: retry after {retry_after:.1f}s")
            except (ValueError, TypeError) as e:
                ...
                if header_name in headers:
                    try:
                        ...
                        if reset_time_str.isdigit():
                            ...
                            reset_datetime = datetime.datetime.fromisoformat(reset_time_str)
                            retry_after = (reset_datetime - datetime.datetime.now().astimezone()).total_seconds()
                        
                        if retry_after > 0:
                            rate_limit_info = RateLimitInfo(retry_after)
                            logger.info(f"Parsed {header_name} for {provider_id}: retry after {retry_after:.1f}s")
                            break
                    except (ValueError, TypeError) as e:
                        logger.debug(f"Failed to parse {header_name} header: {e}")
        ...
        if rate_limit_info:
            ...
            logger.warning(
                f"Rate limit set for provider {provider_id}: "
                f"retry after {rate_limit_info.retry_after:.1f} seconds"
            )
    ...
    def set_rate_limit_manual(self, provider_id: str, retry_after: float):
        ...
        logger.warning(f"Manual rate limit set for provider {provider_id}: retry after {retry_after:.1f}s")
    ...
    def clear_rate_limit(self, provider_id: str):
        ...
        with self._lock:
            if provider_id in self._rate_limits:
                del self._rate_limits[provider_id]
                logger.info(f"Rate limit cleared for provider: {provider_id}")
