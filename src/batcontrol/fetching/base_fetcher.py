...
class BaseFetcher(ABC):
    ...
    def __init__(
        ...
        logger.info("Initialized %s provider (type: %s, cache_ttl: %ss, "
                   "refresh_interval: %ss)", self.get_provider_id(),
                   provider_type, self.cache_ttl, self.refresh_interval)
    ...
    def get_data(self) -> Any:
        ...
        if time_since_update <= self.cache_ttl:
            ...
            if cached_data is not None:
                logger.debug("[%s] Using cached data (age: %.1fs)",
                           provider_id, time_since_update)
                return cached_data

        # Cache miss or expired - fetch new data
        logger.debug("[%s] Cache miss, fetching new data (last update: %.1fs ago)",
                    provider_id, time_since_update)
        ...
        try:
            ...
            logger.info("[%s] Successfully fetched and cached new data", provider_id)
            ...
            cached_data = self.cache_manager.get(self._cache_key)
            if cached_data is not None:
                logger.warning("[%s] Using cached data due to error: %s", provider_id, e)
                return cached_data

            logger.error("[%s] No cached data available, re-raising error", provider_id)
            raise
    ...
    def _fetch_with_error_handling(self) -> Any:
        ...
        try:
            ...
            logger.error("[%s] Network error during fetch: %s", provider_id, e)
            raise
        except Exception as e:
            logger.error("[%s] Unexpected error during fetch: %s", provider_id, e, exc_info=True)
    ...
    def invalidate_cache(self):
        ...
        logger.info("[%s] Cache invalidated", self.get_provider_id())
    ...
    def force_refresh(self) -> Any:
        ...
        logger.info("[%s] Forcing refresh", self.get_provider_id())
