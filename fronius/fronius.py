...
class FroniusWR(InverterBaseclass):
    ...
    def restore_time_of_use_config(self):
        try:
            ...
            logger.error(f'[Fronius] could not restore timeofuse config')
            return
        
        try:
            time_of_use_config=json.loads(time_of_use_config_json)
        except:
            logger.error(f'[Fronius] could not parse timeofuse config from {TIMEOFUSE_CONFIG_FILENAME}')
            return
    ...
    def set_time_of_use(self, timeofuselist):
        ...
        response= self.send_request('/config/timeofuse', method='POST',payload=payload,auth=True)
        response_dict=json.loads(response.text)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if self.login_attempts >=3:
                        logger.info('[Fronius] Login failed 3 times .. aborting')
                        raise RuntimeError('[Fronius] Login failed repeatedly .. wrong credentials?')
                    response = self.login()
                    if (response.status_code==200):
                ...
                logger.error(f"[Fronius] Connection to Inverter failed on {self.address}. Retrying in 120 seconds")
                time.sleep(20)
   ...
   # max_pv_charge_rate (int)   - Maximum power in W that can be used to load the battery from the PV
    ...
    def activate_mqtt(self, api_mqtt_api):  # no type here to prevent the need of loading mqtt_api
        ...
        self.mqtt_api.register_set_callback(self._get_mqtt_topic() + 'max_grid_charge_rate', self.api_set_max_grid_charge_rate, int)
        self.mqtt_api.register_set_callback(self._get_mqtt_topic() + 'max_pv_charge_rate', self.api_set_max_pv_charge_rate, int)
    ...
    def refresh_api_values(self):
        if self.mqtt_api:
            ...
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'usable_capacity', self.get_usable_capacity())
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'max_grid_charge_rate', self.max_grid_charge_rate)
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'max_pv_charge_rate', self.max_pv_charge_rate)
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'min_soc', self.min_soc)
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'max_soc', self.max_soc)
            
            self.mqtt_api.generic_publish(self._get_mqtt_topic() + 'capacity', self.get_capacity())

    def api_set_max_grid_charge_rate(self, max_grid_charge_rate:int):
        if max_grid_charge_rate<0:
            logger.warning(f'[Fronius] API: Invalid max_grid_charge_rate {max_grid_charge_rate}')
            return
        logger.info(f'[Fronius] API: Setting max_grid_charge_rate: {max_grid_charge_rate}W')     
        self.max_grid_charge_rate = max_grid_charge_rate

    def api_set_max_pv_charge_rate(self, max_pv_charge_rate:int):
        if max_pv_charge_rate<0:
            logger.warning(f'[Fronius] API: Invalid max_pv_charge_rate {max_pv_charge_rate}')
            return
        logger.info(f'[Fronius] API: Setting max_pv_charge_rate: {max_pv_charge_rate}W')     
        self.max_pv_charge_rate = max_pv_charge_rate
